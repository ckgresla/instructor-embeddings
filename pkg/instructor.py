# Generate Embeddings w Instructor
# Hard Coded in the Module from- https://github.com/HKUNLP/instructor-embedding
# fetch main model from here with "from pkg import INSTRUCTOR" --> then add desired model name (defaults to 'large')
...
class INSTRUCTOR_Transformer(Transformer):
    ...
    def tokenize(self, texts):
        ...
        #     to_tokenize = [to_tokenize]
            ...
            assert (
                len(texts[0]) == 2
            ), f"The input should have both instruction and input text"
            # if len(texts[0])==3:
            # print('component 3')
            num = len(texts)
            contexts = []
...
# Main Class Subclasses this fellow- https://github.com/UKPLab/sentence-transformers/blob/3e1929fddef16df94f8bc6e3b10598a98f46e62d/sentence_transformers/SentenceTransformer.py#L33
class INSTRUCTOR(SentenceTransformer):
    # Upstream Repo- https://huggingface.co/hkunlp/instructor-large
    # this class initalizes their models from huggingface repos
    ...
    # Helper func for computing embeddings 
    def compute_embedding(
        self,
        input_text: str,
        prompt: str = "Embed the following sentence for semantic similarity: ",
        normalize: bool = False,
    ) -> np.ndarray:
        """
        Given the text to embed & a prompt describing how to represent the text, compute the embedding

        Example:
          * input_text = 'I would like to send an email to Frank, we havent chatted in a while and I wanna catch up'
          * prompt = 'Represent this Task Oriented Query for retrieval: '

          compute_embedding(input_text, prompt) --> returns a numpy array of shape [1, 768]

        Parameters:
          * normalize: bool, whether or not to normalize the embeddings (unit vector, length of one)- https://mathworld.wolfram.com/NormalizedVector.html --> required for 'inner-product' distance metric in milvus
        """
        if prompt[-2:] != ": ":
            prompt += ": "  # examples use this string to end prompt in docs

        inputs = [[prompt, input_text]]  # specific input shape for Model (batch of 1)
        vec: np.ndarray = self.encode(
            inputs, normalize_embeddings=normalize, convert_to_numpy=True
        )  # type: ignore
        return vec
